...
if(isset($_REQUEST['date'])){
   ...
   header('Location:other_bookings_data.php');
		...
		<table cellpadding="0" cellspacing="0" width="100%" class="table table-striped table-bordered" id="example">
		 <thead>
			...
			<th>&nbsp;</th>
			 ...
			 //echo $date." ".$branch_id;
			 //return;
			 $selbk=mysql_query("select * from booking where booked_date='$date' && branch_id='$branch_id' && status='served'")or die(mysql_error());
			 $countbk=0;
...
<?php 
 //echo $date." ".$branch_id;
 //return;
 $selbk=mysql_query("select * from booking where booked_date='$date' && branch_id='$branch_id' && status='canceled'")or die(mysql_error());
 $countbk=0;
 while($valbk=mysql_fetch_array($selbk)){			 
   $countbk++;//counter
   $booking_id=$valbk['booking_id'];
